...
class Reservation extends Component {
  ...
  async componentDidMount() {
    if (storage.getCalendar()) {
      ...
      const rooms = await DataService.getAvailableRoomsByDates(calendar);
      this.setState({ rooms });
      return;
    }
      ...
      <div className="container-reservation">
        <div className="container-description-reservation">
          <div className="select-date">
            {calendar && calendar.startDate && calendar.endDate && (
              <div>
                ...
                <p className="start-parraf">
                  <span className="check-selected">Check-in: </span>
                  <span className="date-selected">
                    {calendar.startDate.displayDate}
                  </span>
                </p>
                <p className="end-parraf">
                  <span className="check-selected">Check-out: </span>
                  <span className="date-selected">
                    {calendar.endDate.displayDate}
                  </span>
                </p>
                <p className="roomType-parraf">
                  <span className="check-selected">Room Type: </span>
                  <span className="date-selected">{calendar.roomType}</span>
                </p>
              </div>
            )}
          </div>
          ...
          <img
            ...
            src="https://www.massalagros.com/wp-content/uploads/2018/05/Gastronomia25.jpg"
          />
          <p className="text-description-reservation">
            Naturae is the first resort 100% ecological of the peninsula. We are
            located in a natural reserve, overlooking the protected park of the
            Serralada Litoral. Our location, together with our 100% sustainable
            operation, make this a unique place in the peninsula.
          </p>
        </div>
                ...
                return (
                  <div className="container-room-firebase">
                    ...
                    <div className="button-price">
                      {!room.available && (
                        <div className="room-nodisponible">
                          ROOM NOT AVAILABLE
                        </div>
                      )}
                      ...
                      <button
                        ...
                        className={
                          !room.available ? "disabled" : "button-deital-rooms"
                        }
                        onClick={() => this.onRoomDetailClicked(room.id)}
                      >
                        {" "}
                        Book now{" "}
                      </button>
